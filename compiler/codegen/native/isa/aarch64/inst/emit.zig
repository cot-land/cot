...
fn finalizeOffset(off: i64, basereg: Reg, result: *MemFinalizeResult) AMode {
    ...
    if (off >= 0 and off <= 32760 and @mod(off, 8) == 0) {
        return AMode{ .unsigned_offset = .{
            .rn = basereg,
            .uimm12 = UImm12Scaled.maybeFromI64(off, Type.I64).?,
        } };
    }
...
/// Emit pre-instructions generated by memFinalize (e.g., MOVZ for large offsets).
fn emitPreInsts(sink: *MachBuffer, finalized: *const MemFinalizeResult) !void {
    for (0..finalized.pre_inst_count) |i| {
        if (finalized.pre_insts[i]) |pre_inst| {
            // Pre-instructions are always mov_wide for loading large offsets
            switch (pre_inst) {
                .mov_wide => |payload| {
                    try sink.put4(encMoveWide(payload.op, payload.rd, payload.imm, payload.size));
                },
                else => {
                    // Only mov_wide should be generated by memFinalize
                    @panic("Unexpected pre-instruction type in memFinalize");
                },
            }
        }
    }
...
/// Handles pseudo addressing modes by calling memFinalize first.
fn emitLoad(sink: *MachBuffer, rd: Writable(Reg), mem: AMode, op: u32, state: *const EmitState) !void {
    // Finalize pseudo addressing modes (sp_offset, slot_offset, etc.)
    const finalized = memFinalize(mem, state);

    // Emit any pre-instructions needed for large offsets
    try emitPreInsts(sink, &finalized);
...
/// Handles pseudo addressing modes by calling memFinalize first.
fn emitStore(sink: *MachBuffer, rd: Reg, mem: AMode, op: u32, state: *const EmitState) !void {
    // Finalize pseudo addressing modes (sp_offset, slot_offset, etc.)
    const finalized = memFinalize(mem, state);
...
pub fn emit(inst: *const Inst, sink: *MachBuffer, emit_info: *const EmitInfo, state: *const EmitState) !void {
    ...
    switch (inst.*) {
            ...
            switch (payload.size) {
                .size64 => {
                    ...
                    if (payload.rm.bits == stackReg().bits) {
                        // Use ADD rd, sp, #0 instead of ORR (register 31 = ZR in ORR, SP in ADD).
        ...
        // Load instructions
        .uload8 => |payload| try emitLoad(sink, payload.rd, payload.mem, 0b0011100001, state),
        .sload8 => |payload| try emitLoad(sink, payload.rd, payload.mem, 0b0011100010, state),
        .uload16 => |payload| try emitLoad(sink, payload.rd, payload.mem, 0b0111100001, state),
        .sload16 => |payload| try emitLoad(sink, payload.rd, payload.mem, 0b0111100010, state),
        .uload32 => |payload| try emitLoad(sink, payload.rd, payload.mem, 0b1011100001, state),
        .sload32 => |payload| try emitLoad(sink, payload.rd, payload.mem, 0b1011100010, state),
        .uload64 => |payload| try emitLoad(sink, payload.rd, payload.mem, 0b1111100001, state),

        // Store instructions
        .store8 => |payload| try emitStore(sink, payload.rd, payload.mem, 0b0011100000, state),
        .store16 => |payload| try emitStore(sink, payload.rd, payload.mem, 0b0111100000, state),
        .store32 => |payload| try emitStore(sink, payload.rd, payload.mem, 0b1011100000, state),
        .store64 => |payload| try emitStore(sink, payload.rd, payload.mem, 0b1111100000, state),
