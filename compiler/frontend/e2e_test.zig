...
const ssa_compile = @import("../ssa/compile.zig");
...
// ============================================================================
// SSA Compile Integration Tests
// These test the full pipeline: source → SSA → expand_calls → decompose →
// schedule → regalloc → stackalloc
// ============================================================================

const CompileTestResult = struct {
    ...
    pub fn deinit(self: *CompileTestResult) void {
        self.pipeline.arena.allocator().free(self.compile_results);
        self.pipeline.arena.allocator().free(self.compile_errors);
        self.pipeline.deinit();
    }

    pub fn hasCompileErrors(self: *const CompileTestResult) bool {
        return self.compile_errors.len > 0;
    }

    pub fn totalSpills(self: *const CompileTestResult) u32 {
        var total: u32 = 0;
        for (self.compile_results) |r| total += r.num_spills;
        return total;
    }
};

fn runFullCompile(backing: std.mem.Allocator, code: []const u8, target: target_mod.Target) !CompileTestResult {
    var pipeline = try runPipeline(backing, code);
    ...
    const allocator = pipeline.arena.allocator();
    var results = std.ArrayListUnmanaged(CompileTestResult.CompileResult){};
    var errors = std.ArrayListUnmanaged(CompileTestResult.CompileError){};

    for (pipeline.ssa_funcs) |ssa_func| {
        // Run full compile pipeline
        var compile_result = ssa_compile.compile(
            allocator,
            ssa_func,
            &pipeline.type_reg,
            target,
        ...
        // Count values across all blocks
        var num_values: usize = 0;
        for (ssa_func.blocks.items) |b| {
            num_values += b.values.items.len;
        }

        try results.append(allocator, .{
            .func_name = ssa_func.name,
            .frame_size = compile_result.stack.frame_size,
            .num_spills = compile_result.regalloc.num_spills,
            .num_blocks = ssa_func.numBlocks(),
    ...
    return .{
        .pipeline = pipeline,
        .compile_results = results.toOwnedSlice(allocator) catch &.{},
        .compile_errors = errors.toOwnedSlice(allocator) catch &.{},
    };
}
...
test "compile: empty function" {
    var result = try runFullCompileNative(std.testing.allocator,
        \\fn empty() void { }
    );
    defer result.deinit();

    try std.testing.expect(!result.pipeline.has_errors);

    // Debug: print SSA function info
    for (result.pipeline.ssa_funcs) |f| {
        std.debug.print("SSA func '{s}': {d} blocks\n", .{ f.name, f.numBlocks() });
        for (f.blocks.items) |b| {
            std.debug.print("  block id={d} kind={s} values={d}\n", .{
                b.id, @tagName(b.kind), b.values.items.len,
            });
        }
    }

    try std.testing.expect(!result.hasCompileErrors());
    try std.testing.expectEqual(@as(usize, 1), result.compile_results.len);

    // Empty function should have minimal frame (just header)
    const r = result.compile_results[0];
    try std.testing.expect(r.frame_size >= 16); // At least FP+LR
    try std.testing.expectEqual(@as(u32, 0), r.num_spills);
}

test "compile: return constant" {
    var result = try runFullCompileNative(std.testing.allocator,
        \\fn answer() i64 { return 42 }
    );
    defer result.deinit();
    ...
    // Constant return needs no spills
    try std.testing.expectEqual(@as(u32, 0), result.compile_results[0].num_spills);
}
...
test "compile: all arithmetic ops" {
    var result = try runFullCompileNative(std.testing.allocator,
        \\fn arith(a: i64, b: i64) i64 {
        \\    var x = a + b
        \\    x = x - b
    ...
    try std.testing.expect(!result.hasCompileErrors());
    const r = result.compile_results[0];
    try std.testing.expect(r.num_values > 0);
}

test "compile: if-else diamond CFG" {
    var result = try runFullCompileNative(std.testing.allocator,
        \\fn max(a: i64, b: i64) i64 {
        \\    if a > b {
        \\        return a
    ...
    try std.testing.expect(!result.hasCompileErrors());
    const r = result.compile_results[0];
    try std.testing.expect(r.num_blocks >= 3); // Diamond needs at least 3 blocks
}
...
test "compile: while loop" {
    ...
    try std.testing.expect(!result.hasCompileErrors());
    const r = result.compile_results[0];
    try std.testing.expect(r.num_blocks >= 2); // Loop needs at least header + body
}
...
test "compile: multiple functions" {
    var result = try runFullCompileNative(std.testing.allocator,
        ...
        \\fn sextuple(x: i64) i64 { return double(triple(x)) }
    );
    defer result.deinit();

    try std.testing.expect(!result.hasCompileErrors());
    try std.testing.expectEqual(@as(usize, 3), result.compile_results.len);
}

test "compile: many parameters (register pressure)" {
    var result = try runFullCompileNative(std.testing.allocator,
    ...
    try std.testing.expect(!result.hasCompileErrors());
    // With 8 params, may need stack space
    const r = result.compile_results[0];
    try std.testing.expect(r.frame_size >= 16);
}

test "compile: nested loops" {
    var result = try runFullCompileNative(std.testing.allocator,
        \\fn nested() i64 {
        \\    var sum = 0
        \\    var i = 0
...
test "compile: complex control flow" {
    ...
    try std.testing.expect(!result.hasCompileErrors());
    const r = result.compile_results[0];
    try std.testing.expect(r.num_blocks >= 5); // Complex CFG
}
...
test "compile: pointer operations" {
    var result = try runFullCompileNative(std.testing.allocator,
        \\fn swap(a: *i64, b: *i64) void {
        \\    var tmp = a.*
        \\    a.* = b.*
...
test "compile: struct operations" {
    var result = try runFullCompileNative(std.testing.allocator,
        \\struct Point { x: i64, y: i64 }
    ...
    try std.testing.expect(!result.hasCompileErrors());
    try std.testing.expectEqual(@as(usize, 2), result.compile_results.len);
}
