...
//===----------------------------------------------------------------------===//
// cir.generic_apply — custom parse/print + verifier
//===----------------------------------------------------------------------===//

/// Parse: cir.generic_apply @fn(%a, %b) subs ["T" = i32] : (i32, i32) -> i32
ParseResult GenericApplyOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  // Parse subs ["T" = i32, "U" = f64]
  llvm::SmallVector<Attribute> subsKeys;
  llvm::SmallVector<Attribute> subsTypes;
  ...
  if (failed(parser.parseOptionalRSquare())) {
    do {
      ...
      if (parser.parseString(&key) || parser.parseEqual() ||
        ...
        return failure();
      subsKeys.push_back(StringAttr::get(parser.getContext(), key));
      subsTypes.push_back(TypeAttr::get(valType));
    } while (succeeded(parser.parseOptionalComma()));
    if (parser.parseRSquare())
      return failure();
  }
  result.addAttribute("subs_keys", ArrayAttr::get(parser.getContext(), subsKeys));
  result.addAttribute("subs_types", TypeAttr::get(
      parser.getBuilder().getNoneType())); // placeholder
  ...
  // Resolve operands
  for (unsigned i = 0; i < operands.size(); i++) {
    Type opType = (i < argTypes.size()) ? argTypes[i] : argTypes.back();
    if (parser.resolveOperand(operands[i], opType, result.operands))
      return failure();
  }

  // Store subs_types as ArrayAttr of TypeAttr
  result.attributes.erase("subs_types");
  result.addAttribute("subs_types",
      ArrayAttr::get(parser.getContext(), subsTypes));

  return success();
}

void GenericApplyOp::print(OpAsmPrinter &p) {
  p << " " << getCallee() << "(";
  p.printOperands(getOperands());
  p << ") subs [";
  auto keys = getSubsKeys();
  auto types = getSubsTypes();
  for (size_t i = 0; i < keys.size(); i++) {
    if (i > 0) p << ", ";
    p << "\"" << llvm::cast<StringAttr>(keys[i]).getValue() << "\" = "
      << llvm::cast<TypeAttr>(types[i]).getValue();
  }
  p << "] : (";
